				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<textarea name="aloow_name" id="" cols="30" rows="2" class="form-control" required></textarea>
							</div>
							<div class="form-group">
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$allowances = $conn->query("SELECT * FROM allowance order by allow_id asc");
								...
								<tr>
									...
									<td class="">
										 <p>Name: <b><?php echo $row['aloow_name'] ?></b></p>
										 <p class="truncate"><small>Description: <b><?php echo $row['description'] ?></b></small></p>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_allowances" type="button" data-id="<?php echo $row['allow_id'] ?>" data-allowance="<?php echo $row['aloow_name'] ?>" data-description="<?php echo $row['description'] ?>" >Edit</button>
										<button class="btn btn-sm btn-danger delete_allowances" type="button" data-id="<?php echo $row['allow_id'] ?>">Delete</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('.edit_allowances').click(function(){
		...
		cat.get(0).reset()
		cat.find("[name='allow_id']").val($(this).attr('data-id'))
		cat.find("[name='aloow_name']").val($(this).attr('data-allowance'))
	...
	function delete_allowances($allow_id){
		start_load()
		$.ajax({
